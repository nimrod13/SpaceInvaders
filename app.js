document.addEventListener("DOMContentLoaded", () => {
  ...
  function shootLaser() {
    ...
    function moveLaser() {
      ...
      let hitByLaser = invaders.find(
        (invaderRelativePosition) =>
          invaderRelativePosition + invadersPosition === laserPosition
      );
  ...
  function invadersTouchingRightEdge() {
    return invaders.some(
      (relativePosition) => (relativePosition + invadersPosition) % 15 === 14
    );
  }

  function invadesTouchingLeftEdge() {
    return invaders.some(
      (relativePosition) => (relativePosition + invadersPosition) % 15 === 0
    );
  }
  ...
  function checkGameOver() {
    if (
      invaders.some(
        (realtivePosition) =>
          realtivePosition + invadersPosition === playerPosition
      )
    ) {
